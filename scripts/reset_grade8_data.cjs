/**
 * scripts/reset_grade8_data.cjs
 * 重置初二(grade8)身份注册的所有用户数据、答题记录、报告数据
 *
 * 删除顺序（按外键依赖）:
 *   1. report_feedback  (依赖 reports)
 *   2. retest_intents   (依赖 students)
 *   3. reports          (依赖 sessions)
 *   4. answer_events    (依赖 answer_records / sessions)
 *   5. answer_records   (依赖 sessions)
 *   6. test_sessions    (依赖 students)
 *   7. students         (依赖 access_codes)
 *   8. access_codes     (identity = 'grade8')
 *
 * 运行: node scripts/reset_grade8_data.cjs
 */
...
async function deleteByFilter(table, filter) {
  // PostgREST DELETE with filter: /table?column=eq.value
...
async function main() {
  console.log('========================================================');
  console.log('  初二(grade8) 数据重置脚本');
  console.log('  目标: 删除所有用户、答题记录、报告数据');
  console.log('========================================================\n');

  // Step 1: 查找所有 grade8 的 access_codes
  console.log('[Step 1] 查找初二身份的 access_codes...');
  ...
  // Step 4: 查找关联的 reports (reports 通过 student_id 关联，不是 session_id)
  console.log('\n[Step 4] 查找关联的 reports...');
  const reports = studentIds.length > 0
    ? await selectByFilter('reports', 'id,student_id,sku_code', `student_id=in.(${studentIdList})`)
    : [];
  console.log(`  找到 ${reports.length} 条 reports 记录`);

  const reportIds = reports.map(r => r.id);
  const reportIdList = reportIds.length > 0 ? reportIds.join(',') : '0';
  ...
  // Step 5: 删除 report_feedback
  if (reportIds.length > 0) {
    console.log('[Step 5] 删除 report_feedback...');
    const count = await deleteByFilter('report_feedback', `report_id=in.(${reportIdList})`);
    console.log(`  已删除 ${count} 条 report_feedback`);
  } else {
    console.log('[Step 5] 跳过 report_feedback（无关联数据）');
  }

  // Step 6: 删除 retest_intents
  if (studentIds.length > 0) {
    console.log('\n[Step 6] 删除 retest_intents...');
    try {
      const count = await deleteByFilter('retest_intents', `student_id=in.(${studentIdList})`);
      console.log(`  已删除 ${count} 条 retest_intents`);
    } catch (e) {
      console.log(`  retest_intents 删除跳过（表可能不存在或无数据）: ${e.message}`);
    }
  } else {
    console.log('\n[Step 6] 跳过 retest_intents（无关联数据）');
  }

  // Step 7: 删除 reports (通过 student_id 删除)
  if (studentIds.length > 0) {
    console.log('\n[Step 7] 删除 reports...');
    const count = await deleteByFilter('reports', `student_id=in.(${studentIdList})`);
    console.log(`  已删除 ${count} 条 reports`);
  } else {
    console.log('\n[Step 7] 跳过 reports（无关联数据）');
  }

  // Step 8: 删除 answer_events
  if (sessionIds.length > 0) {
    ...
    try {
      ...
      // answer_events 可能用 record_id 关联而非 session_id，尝试通过 records 查找
      console.log(`  answer_events 通过 session_id 删除失败，尝试通过 answer_records 关联...`);
      try {
        const records = await selectByFilter('answer_records', 'id', `session_id=in.(${sessionIdList})`);
        if (records.length > 0) {
          const recordIds = records.map(r => r.id).join(',');
          const count = await deleteByFilter('answer_events', `record_id=in.(${recordIds})`);
          console.log(`  已删除 ${count} 条 answer_events (via record_id)`);
        } else {
          console.log('  无关联 answer_events 数据');
        }
      } catch (e2) {
        console.log(`  answer_events 删除跳过: ${e2.message}`);
      }
    }
  } else {
    console.log('\n[Step 8] 跳过 answer_events（无关联数据）');
  }

  // Step 9: 删除 answer_records
  if (sessionIds.length > 0) {
    console.log('\n[Step 9] 删除 answer_records...');
    const count = await deleteByFilter('answer_records', `session_id=in.(${sessionIdList})`);
    console.log(`  已删除 ${count} 条 answer_records`);
  } else {
    console.log('\n[Step 9] 跳过 answer_records（无关联数据）');
  }

  // Step 10: 删除 test_sessions
  if (studentIds.length > 0) {
    console.log('\n[Step 10] 删除 test_sessions...');
    const count = await deleteByFilter('test_sessions', `student_id=in.(${studentIdList})`);
    console.log(`  已删除 ${count} 条 test_sessions`);
  } else {
    console.log('\n[Step 10] 跳过 test_sessions（无关联数据）');
  }
  ...
  if (accessCodeIds.length > 0) {
    ...
    console.log('\n[Step 11] 跳过 students（无关联数据）');
  }

  // Step 12: 删除 access_codes
  console.log('\n[Step 12] 删除 access_codes (identity=grade8)...');
  const count = await deleteByFilter('access_codes', 'identity=eq.grade8');
  ...
  const remainingAC = await selectByFilter('access_codes', 'id,code,identity', 'identity=eq.grade8');
  console.log(`[验证] 剩余 grade8 access_codes: ${remainingAC.length} 条`);
  ...
  console.log('\n========================================================');
  console.log('  数据重置完成！');
  console.log('  初二身份的所有用户数据、答题记录、报告数据已清空。');
